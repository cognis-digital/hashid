"""Native cognis-connect emit for hashid — forward findings to any platform.

Maps hashid's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    hashid ... --format json | hashid-emit --to stix
    hashid-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('input_length', None)
        out['title'] = rec.get('value', '')
        out['severity'] = 'info'
        out['type'] = 'hash'
        out['description'] = 'Hash type identified'
        out['tags'] = ['hash']
